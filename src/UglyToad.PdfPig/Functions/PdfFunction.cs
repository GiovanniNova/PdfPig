namespace UglyToad.PdfPig.Functions
{
    ...
    public abstract class PdfFunction
    {
        ...
        /// <summary>
        /// Create the correct <see cref="PdfFunction"/> based on the function union.
        /// </summary>
        /// <param name="function">The function, as an union.</param>
        /// <returns>The Function object, never null.</returns>
        public static PdfFunction Create(Union<DictionaryToken, StreamToken> function)
        {
            if (function.TryGetFirst(out var dict))
            {
                return Create(dict);
            }
            else if (function.TryGetSecond(out var stream))
            {
                return Create(stream);
            }

            return Create(NameToken.Identity);
        }

        /// <summary>
        /// Create the correct <see cref="PdfFunction"/> based on the <see cref="IToken"/> function.
        /// </summary>
        /// <param name="function">The function <see cref="StreamToken"/> or <see cref="DictionaryToken"/>.</param>
        /// <returns>The Function object, never null.</returns>
        public static PdfFunction Create(IToken function)
        {
            ...
            if (_baseDic is not DictionaryToken functionDictionary)
            {
                throw new IOException("Error: Function must be a Dictionary, but is " +
                        (_baseDic == null ? "(null)" : _baseDic.GetType().Name));
            }

            if (functionDictionary.ContainsKey(NameToken.Filter) || functionDictionary.ContainsKey(NameToken.F))
            {
                throw new IOException("Function should be decoded");
            }
