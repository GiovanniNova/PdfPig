namespace UglyToad.PdfPig.Tests.Functions.Type4
{
    ...
    public class OperatorsTests
    {
        ...
        public void Add()
        {
            ...
            double floatResult = Convert.ToDouble(context.GetStack().Pop());
            Assert.Equal((long)2 * (long)int.MaxValue - (long)4, floatResult, 1);

            Assert.Empty(context.GetStack());
        }
        ...
        public void Cvr()
        {
            ...
            Assert.True(context.GetStack().Pop() is double, "Expected a real as the result of 'cvr'");
            Assert.True(context.GetStack().Pop() is int, "Expected an int from an int literal");
        }
